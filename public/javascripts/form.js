...
HTML5FORM.Form = Class.create({
    ...
    initialize: function(options) {
        ...
        self._createOptionsObjectFromArguments(options); // Mixin options object with default options, allows for customisation

        if(!self.options.formId) {
            throw 'MissingArgument - formId is required';
        }
...
console.log(self.form, self.options.formId);
        ...
        self.form.select('input:not([type="submit"])').each(function(el) { // select inputs from within the form that should be validated
            self._registerFields(el); // create a fields object and add it to an array of fields
        });

        self.form.onsubmit = self.options.onSubmit.bind(self); // bind the submithandler to the forms submit event

        self.form.observe('form:validationSuccess', self.options.onValidationSuccess); // bind the onValidation success handler to a custom event
    },
    ...
    _addNoValidateAttr: function() {
        ...
        self.form.writeAttribute('novalidate', 'novalidate');
    },
